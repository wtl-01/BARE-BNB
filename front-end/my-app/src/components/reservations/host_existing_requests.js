...
const BookingCard = ({
  ...
  const [terminate_button, setTerminateButtonDisplay] = useState("Decline");
      ...
      fetch(`${baseURL}/bookings/${booking_id}/edit/`, {
        ...
        .then((response) => {
          if (!response.ok) {
            ...
            console.log("Success");
            toast.success(
              "Reservation has been terminated successfully. Please refresh the page to see changes."
            );
            setTerminateButtonDisplay("Declined");
          }
        })
        .catch((err) => {
          console.log(err);
...
function HostExisting() {
  ...
  const fetchData = async () => {
    const response1 = await fetch(
      `${baseURL}/bookings/?type=host&state=Approved&page=${currentPage}`,
      {
        method: "GET",
        headers: {
          Authorization: `Bearer ${accessToken}`,
