...
const BookingCard = ({
  ...
  const [cancel_button, setCancelButtonDisplay] = useState("Cancel");
      ...
      fetch(`${baseURL}/bookings/${booking_id}/edit/`, {
        ...
        .then((response) => {
          if (!response.ok) {
            ...
            console.log("Success");
            toast.success(
              "Reservation has been cancelled successfully. Please refresh the page to see changes."
            );
            setCancelButtonDisplay("Cancelled");
          }
        })
        .catch((err) => {
          console.log(err);
...
function HostCancellation() {
  ...
  const fetchData = async () => {
    const response1 = await fetch(
      `${baseURL}/bookings/?type=host&state=RequestCancel&page=${currentPage}`,
      {
        method: "GET",
        headers: {
          Authorization: `Bearer ${accessToken}`,
